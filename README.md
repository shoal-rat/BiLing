...
标记，只升不降），再叠加按上下文单独校准的模型门控。150 条留出集配对测试
（两边看到完全相同的前文）：笔灵 76.0%，Apple 拼音 77.3%，95% 置信区间
[−7.3, +4.7] —— 统计上不可区分；在 43 条专测前文歧义的对照集上笔灵 79.1%
对 67.4%（n 太小，置信区间触零，不下定论）。冷启动 Apple 仍然领先，如上表。
方法与命令见 `Docs/results/apple-comparison-context.txt`。
